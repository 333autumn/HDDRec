...
LOG_PATH = join(world.ROOT_PATH, 'log')
if not os.path.exists(LOG_PATH):
    os.makedirs(LOG_PATH, exist_ok=True)

log_file = join(LOG_PATH, f"{world.model_name}_{world.dataset}_{time.strftime('%Y%m%d_%H%M%S')}.log")
...
def main():
    ...
    try:
        ...
        if isinstance(sys.stdout, Logger):
            print("Training completed. Log saved.")
            sys.stdout.close()
